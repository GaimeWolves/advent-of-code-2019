...
int main(int argc, char** argv)
{
	...
	while(std::getline(iss, token, ','))
	{
		...
		stepsA.push_back(stepsA[stepsA.size() - 1] + delta);
	}
	...
	while(std::getline(iss, token, ','))
	{
		...
		for (int i = 1; i < wireA.size(); i++)
		{
			...
			{
				int actualStepsB = stepsB + ((x1 == x2) ? (std::abs(p.second - y1)) : (std::abs(p.first - x1)));
				int actualStepsA = stepsA[i - 1] + ((x3 == x4) ? (std::abs(p.second - y3)) : (std::abs(p.first - x3)));

				if (actualStepsA + actualStepsB < best)
					best = actualStepsA + actualStepsB;
			}
		}
